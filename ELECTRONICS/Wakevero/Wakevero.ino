...
#define UART2_TX 17
#define UART2_RX 16
#define SLEEP_TIME 3600e6  // Sleep for 1 hour bursts then go to sleep again
...
RTC_DS3231 rtc;
Adafruit_BME280 bme;
//Adafruit_SH1106G display = Adafruit_SH1106G(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, OLED_RESET);//1.3 inch OLED display
Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, OLED_RESET); //o.96 inch OLED display
...
//the days files which will be assigned in the setup function
String WeatherFile;
String WeightFile;
...
void setupFiles() {
  // Weight file
  String filenameweight = getFilename("/DataWeight");
  WeightFile = filenameweight;
  //dataweight = SD.open("/DataWeight.txt", FILE_WRITE);
  dataweight = SD.open(filenameweight, FILE_WRITE);
  if (dataweight) {
  ...
  // Weather data
  String filenameweather = getFilename("/DataWeather");
  WeatherFile = filenameweather;
  //dataweather = SD.open("/DataWeather.txt", FILE_WRITE);
  dataweather = SD.open(filenameweather, FILE_WRITE);
  if (dataweather) {
...
String readtimeData2() {
  ...
  // Combine the time stamp into one string in the format YYYY_MM_DD
  String timestamp2 = String(YY) + "_" + String(MMM) + "_" + String(DD);
  return timestamp2;
}
...
float dynamic_weight_analysis(std::vector<int> combined_scale_readings, float calibration_factor) {
  ...
  //The algorithm is being implemented in the next version of the code
...
void writeWeightData() {
  ...
  dataweight = SD.open(WeightFile, FILE_APPEND);
  if (dataweight) {
...
void writeWeatherData() {
  ...
  dataweather = SD.open(WeatherFile, FILE_APPEND);
  if (dataweather) {
